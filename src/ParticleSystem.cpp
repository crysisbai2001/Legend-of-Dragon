...
void GravityForce::set(const ofVec3f& gravity) {
...
void ImpulseRadialForce::updateForce(Particle * particle) {
	...
	particle->forces += dir.getNormalized() * magnitude * ofRandom(0.7, 1);
}
...
void RadialForce::updateForce(Particle* particle) {
	dir = ofVec3f(ofRandom(-1, 1), ofRandom(-0.01, 0.01), ofRandom(-1, 1));
	particle->forces += dir.getNormalized() * magnitude * ofRandom(0.7, 1);
}

void RadialForce::set(float magnitude) {
	this->magnitude = magnitude;
}
...
void CyclicForce::updateForce(Particle* particle) {
	ofVec3f force = particle->position.getPerpendicular(ofVec3f(particle->position.x, 0, particle->position.z)) / (ofGetElapsedTimef() * 2) ;
	particle->forces += force.getNormalized() * magnitude;
}

void CyclicForce::set(float magnitude) {
	this->magnitude = magnitude;
}